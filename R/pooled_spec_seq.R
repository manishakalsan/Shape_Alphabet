# pool the binding sites of all the TFs from encode together and then 
# make one classifier for all TFs to check if the high shape accuracies 
# are coming because of some factor that is not TF specific but TF binding 
# mechanism specific for eg. open chromatin, if nucleosome occupancy is 
# low, TFs binding will be higher
# in this version, we are trying to use a model trained on the pooled data 
# of binding sites (the training set) and test it on a TF-specific binding 
# site and random sites data

# SEQUENCE
# pooled training TF spec test
...
# encode both datasets and prepare pooled set of tfbs and random background for training
...
# prepare tf specific test sets and apply model developed for pooled data
tf_model_res <- lapply(1:length(list_TFs), function(x){
     tf_spec_test_data <- seq_tf_test_data(list_TFs[x], 20) 
     
     # prepare test data
     X_test <- as.matrix(tf_spec_test_data[,-ncol(tf_spec_test_data)])
     y_test <- as.matrix(tf_spec_test_data[,ncol(tf_spec_test_data)])
     ...
     results<-as.data.frame(c(list_TFs[x], rmse, mae, accuracy, auc_roc))
     t(results)
     
})
tf_model_res_df <- do.call(rbind, tf_model_res)

colnames(tf_model_res_df) <- c("TF", "rmse", "mae", "accuracy", "auc")
write.table(tf_model_res_df, "pooled5k_training_tf_spec_test_nuc_binary", row.names = F,
            quote = F, sep="\t")
