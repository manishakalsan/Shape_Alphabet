...
create_merged_data_seq <- function(seq_list, TF_labels, sequence_length=20){
  datasets_enc <- lapply(1:length(seq_list), function(x){
    seq_data <- as.data.frame(get(seq_list[x]))
    print(dim(seq_data))
    # remove sequences with N
...
create_merged_data_shape <- function(seq_list, TF_labels, cluster_names, sequence_length=20){
  encode_vec_length <- length(cluster_names)*(sequence_length-4)
  datasets_enc <- lapply(1:length(seq_list), function(x){
    seq_data <- as.data.frame(get(seq_list[x]))
    print(dim(seq_data))
...
combine_shape_seq <- function(seq_list, tf_name, seq_labels, clustering_method, sequence_length){
     for(i in 1:length(seq_list)){
          seq_data <- as.data.frame(get(seq_list[x]))
          shape_clusters <- assign_clusters(seq_data, tf_name, "tmp")
          
          shape_data <- as.data.frame(shape_clusters[,clustering_method+2])
          
          cluster_names<-clusters[1:num_clust[clustering_method]]
          strings_list <- c("seq_data", "shape_data")
          
          encoded_features <- lapply(1:length(strings_list), function(j){
               strings_data <- as.data.frame(get(strings_list[y]))
               encoded <- c()
               if(j==1){
                    encoded <- mclapply(1:nrow(strings_data), function(y){
                         nuc_coded <- oneHot(strings_data[y,])
                         c(nuc_coded)
                    }, mc.cores = 20)
                    encoded <- do.call(rbind, encoded)
                    coln<-c()
                    ...
                    colnames(encoded) <- c(coln, "TFlabel")
                    
               } else {
                    encoded <- mclapply(1:nrow(strings_data), function(y){
                         nuc_coded <- oneHot(strings_data[y,], nucleotide = cluster_names)
                         c(nuc_coded)
                    }, mc.cores = 20)
                    encoded <- do.call(rbind, encoded)
                    coln<-c()
               ...
               encoded <- cbind(encoded, rep(seq_labels[x], nrow(encoded)))
          })
          encoded_features <- do.call(cbind, encoded_features)
...
for(x in 1:nrow(tf_names)){
     ...
     seq_merged_data <- create_merged_data_seq(tf_seq_list, TF_labels)
     ...
     for(m in 4:21){
          ...
          shape_merged_data <- create_merged_data(tf_seq_list, TF_labels, cluster_names, 20)
