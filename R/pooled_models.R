...
pooled_tfbs <- read.table("pooled_tfbs_500", header = F)
pooled_shuffled <- read.table("pooled_shuffled_500", header = F)

tf_seq_list <- c("pooled_tfbs", "pooled_shuffled")
TF_labels <- c(1,0)

#sequence model
merged_data <- create_merged_data_seq(tf_seq_list, TF_labels, nuc, 51, num_cores)

pdf("pooled_seq_models.pdf")

mod_res <- seq_model("pooled", merged_data, num_cores)

dev.off()

write.table(mod_res, "seq_pooled_parameterization", row.names = F, quote=F)

# create shape alphabets
bs_file_name <- "pooled_tfbs_shape"
r_file_name <- "pooled_shuffled_shape"

nsites = as.numeric(100000) # because nsites in pooled data is less than this
# generate shape alphabets
assign_clusters(pooled_tfbs, "pooled_tfbs", paste(bs_file_name), nsites, num_cores)
assign_clusters(pooled_shuffled, "pooled_shuffled", paste(r_file_name), nsites, num_cores)

# read shape alphabet files
...
#prepare merged data for tfbs and shuffled
merged_data <- create_merged_data_shape(tf_seq_list, TF_labels, clusters, 51, num_cores)

pdf("shape_models_pooled.pdf")

mod_res <- shape_model("pooled", merged_data, num_cores)

dev.off()

write.table(mod_res, "shape_pooled_parameterization", row.names = F, quote=F)
