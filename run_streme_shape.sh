...
for TFname in "${tfnames[@]}"; do
    ...
    if [[ -f "$fasta_file" ]]; then
        ...
        output_dir="${TFname}_streme_shape"

        # Run meme command with the specified parameters
        streme -alph shape_alphabet -oc "$output_dir" -minw 5 -maxw 15 -nmotifs 3 -p "$fasta_file" -n "$shuffled_file"
...
for TFname in "${tfnames[@]}"; do
    ...
    if [[ -f "$fasta_file" ]]; then
        # Define the output directory
        output_dir="${TFname}_streme_shape_cd"

        # Run meme command with the specified parameters
        streme -alph shape_alphabet -oc "$output_dir" -minw 5 -maxw 15 -nmotifs 3 -objfun cd -p "$fasta_file" -n "$shuffled_file"
