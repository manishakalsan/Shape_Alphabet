...
for TFname in "${tfnames[@]}"; do
    ...
    if [[ -f "$fasta_file" ]]; then
        ...
        output_dir="${TFname}_random_shape_meme"

        # Run meme command with the specified parameters
        meme -alph random_shape_alphabet -nmotifs 3 -minw 5 -maxw 15 -mod zoops -maxsize 100000000 -oc "$output_dir" "$fasta_file"
...
for TFname in "${tfnames[@]}"; do
    ...
    if [[ -f "$fasta_file" ]]; then
        # Define the output directory
        output_dir="shuffled_${TFname}_random_shape_meme"
